                        ...
                        <div className="relative pt-6 px-4 sm:px-6 lg:px-8">
                            <nav aria-label="Global" className="relative flex items-center justify-between sm:h-10 lg:justify-start">
                                ...
                                <div className="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
                                    {navigation.map(item => (
                                        <a key={item.name} className="font-medium text-gray-500 hover:text-gray-900" href={item.href}>
                                            {item.name}
                                        </a>
                                    ))}
                            ...
                            <Popover.Panel focus className="absolute z-10 top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden">
                                <div className="rounded-lg shadow-md bg-white ring-1 ring-black ring-opacity-5 overflow-hidden">
                                    <div className="px-5 pt-4 flex items-center justify-between">
                                        <div>
                                            <img alt="" className="h-8 w-auto" src="https://tailwindui.com/img/logos/workflow-mark-indigo-600.svg" />
                                        </div>
                                        <div className="-mr-2">
                                            <Popover.Button className="bg-white rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
                                    ...
                                    <div className="px-2 pt-2 pb-3 space-y-1">
                                        {navigation.map(item => (
                                            <a key={item.name} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50" href={item.href}>
                                                {item.name}
                                            </a>
                                        ))}
                                    </div>
                                    <a className="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100" href="/login">
                                        Log in
                                    </a>
                                </div>
                            </Popover.Panel>
